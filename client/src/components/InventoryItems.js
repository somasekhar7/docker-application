...
function InventoryManagement() {
  ...
  const fetchInventoryItems = async () => {
    try {
      const response = await axios.get(
        "http://ec2-3-134-99-140.us-east-2.compute.amazonaws.com:5005/inventory"
      );
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      await axios.post(
        "http://ec2-3-134-99-140.us-east-2.compute.amazonaws.com:5005/inventory/add-item",
        formDataToSend
      );
      setFormData({
        itemName: "",
  ...
  const handleUpdateItem = async () => {
    ...
    try {
      await axios.put(
        `http://ec2-3-134-99-140.us-east-2.compute.amazonaws.com:5005/inventory/update-item/${editingItem._id}`,
        {
  ...
  const handleDeleteItem = async () => {
    ...
    try {
      await axios.delete(
        `http://ec2-3-134-99-140.us-east-2.compute.amazonaws.com:5005/inventory/delete-item/${itemIdToDelete}`
      );
      fetchInventoryItems();
          ...
          {inventoryItems.map((item, index) => (
            <Col key={item._id}>
              <Card style={{ width: "18rem" }}>
                <Card.Img
                  variant="top"
                  src={`http://ec2-3-134-99-140.us-east-2.compute.amazonaws.com:5005/uploads/${item.itemImage}`}
